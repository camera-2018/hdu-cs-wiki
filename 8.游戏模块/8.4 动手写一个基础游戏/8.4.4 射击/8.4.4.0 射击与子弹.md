...
1. 找到子弹预制体 Bullet
2. 挂载新建类 Bullet 并实现
3. 将 Bullet 的**Box Collider 2D**中的**isTrigger**打开
...
1. 打开菜单栏的 Edit $\rightarrow$ Project Settings $\rightarrow$ Phythics 2D,修改碰撞关系：
2. 关闭 Bullet 与 Player 的碰撞
3. 关闭 Enemy 之间的碰撞
...
当碰撞体为触发器，且进入触发范围时执行。为触发器的碰撞体不会产生真实的物理碰撞效果，仅作碰撞检测。

相关链接：[OnTriggerEnter2D](https://docs.unity.cn/cn/current/ScriptReference/Collider2D.OnTriggerEnter2D.html)
