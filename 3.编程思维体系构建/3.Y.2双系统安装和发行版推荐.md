...
哈利波特上网可以直接通过 Flatpak 安装哈利波特的猫头鹰，自己设置订阅和开机自启动，接着在系统代理里面开启代理并且设置端口。
...
# 安装 猫头 for Windows，可能还是有点慢。这行命令肯定不能用，你要自行去 flathub 上面找那个软件
flatpak install flathub io.github.<不知道是谁>.<猫头>_for_windows
```

如果 flathub 实在很慢，睡前安装，挂一晚上。
