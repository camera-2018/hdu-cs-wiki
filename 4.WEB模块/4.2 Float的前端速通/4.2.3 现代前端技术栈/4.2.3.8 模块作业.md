...
我们选择了一个电子书商城作为作业。这里你需要先部署一个 json-server 作为基础的后端，将后端运行起来后在这个后端的基础上进行前端开发。
...
2. **全局状态管理**：
   - 使用 Zustand 管理购物车中的书籍状态，计算总价和处理添加/删除操作。
   - 管理用户登录状态（可选，自行拓展 json-server）。

3. **服务端请求**：
   - 从后端 API 获取电子书列表、书籍详情（可以使用 Mock 数据或 JSON 服务器）。
   - 提交用户购买信息到服务器，并模拟返回购买结果。

4. **资源**：
   - 书籍的封面图片资源，使用公共 API 或本地图片。
...
为了使用 `json-server` 来 host 你需要的电子书数据和生成 API，我们可以按照以下步骤进行设置并扩展电子书列表至 15 本。
